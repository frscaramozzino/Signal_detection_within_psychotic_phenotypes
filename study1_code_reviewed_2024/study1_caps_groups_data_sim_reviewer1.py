...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 18 16:16:57 2025
...
observed_quantiles_caps=get_quantiles(data)
#get simulated quantiles

#Create simulated data for parameter recovery

trials_per_level = 15
subjs_per_bin=int((n_participants+1)/2)
...
for i in sample:
    ...
    pd.concat(simulated_data).to_csv('simulated_data_capsgroup_2025_reviewer1.csv')
    
mean_simulated_quantiles_caps=get_mean_sim_quantiles(simulated_data).assign(source = "simulated")

mean_simulated_quantiles_caps.to_csv('mean_simulated_quantiles_caps_2025_reviewer1.csv')
#%%
...
simulated_data_capsgroup=pd.read_csv('simulated_data_capsgroup_2025_reviewer1.csv')

mean_simulated_quantiles_caps=get_quantiles(simulated_data_capsgroup)
...
#get observed quantiles
data['coherence']=data['coherence'].astype('string')
observed_quantiles_caps=get_condition_quantiles(data,'coherence','caps_group')
#get observed quantiles
observed_quantiles_caps=get_quantiles(data)
#get simulated quantiles
observed_quantiles_caps=observed_quantiles_caps.assign(source = "observed")

mean_simulated_quantiles_caps=mean_simulated_quantiles_caps.drop(columns=['quantile','source'])
observed_quantiles_caps=observed_quantiles_caps.drop(columns=['quantile','source'])
...
rt_act=data[['rt','response','coherence','caps_group']]
...
mse_1 = str(mean_squared_error(observed_quantiles_caps['q_correct'], mean_simulated_quantiles_caps['q_correct']).round(3))
mse_1="".join([mse,mse_1])
mse_0 = str(mean_squared_error(observed_quantiles_caps['q_incorrect'], mean_simulated_quantiles_caps['q_incorrect']).round(3))
...
fig.add_trace(go.Scatter(
    ...
    y=observed_quantiles_caps['q_correct'],
    name='Correct'),
col=1, row=1)

fig.add_trace(go.Scatter(
    x=mean_simulated_quantiles_caps['q_incorrect'],
    y=observed_quantiles_caps['q_incorrect'],
    name='Incorrect'),
col=2, row=1)
...
fig.add_shape(type='line',
                x0=0,
                y0=0,
                x1=mean_simulated_quantiles_caps['q_correct'].max(),
                y1=mean_simulated_quantiles_caps['q_correct'].max(),
                line=dict(color='black'),
                row=1,
                col=1)
fig.add_shape(type='line',
                x0=0,
                y0=0,
                x1=observed_quantiles_caps['q_incorrect'].max(),
                y1=observed_quantiles_caps['q_incorrect'].max(),
                line=dict(color='black'),
                row=1,
                col=2)
